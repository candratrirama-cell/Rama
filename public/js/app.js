...
// AUTOPLAY STATE
// ============================================================
let autoplayEnabled = JSON.parse(localStorage.getItem('yanime_autoplay') || 'false');

function toggleAutoplay() {
  autoplayEnabled = !autoplayEnabled;
  localStorage.setItem('yanime_autoplay', JSON.stringify(autoplayEnabled));
...
function renderSettingPage(user) {
  ...
  if (!user) {
    wrap.innerHTML = `
      ...
      <div class="setting-card">
        ...
        <div class="setting-info"><div class="setting-title">Tentang YAnime</div><div class="setting-desc">Sakura Galaxy Edition v3.0</div></div>
        <span class="setting-badge">v3.0</span>
      </div>
    `;
    return;
  }
  const savedName = localStorage.getItem('yanime_displayname') || user.displayName || 'YAnime User';
  ...
  wrap.innerHTML = `
    <div class="profile-card">
      ...
      <div class="profile-sub">MEMBER YAnime</div>
    </div>
    <div class="setting-card" onclick="clearHistory()">
      <div class="setting-icon">${ICONS.trash}</div>
      <div class="setting-info"><div class="setting-title">Hapus Riwayat</div><div class="setting-desc">Bersihkan semua riwayat nonton</div></div>
...
function openEditProfileModal() {
  ...
  const savedName = localStorage.getItem('yanime_displayname') || (user ? user.displayName : '') || '';
...
async function saveProfile() {
  ...
  try {
    ...
    localStorage.setItem('yanime_displayname', nameVal.trim());
...
async function doRegister() {
  ...
  try {
    ...
    localStorage.setItem('yanime_displayname', name);
...
// ============================================================
const HISTORY_KEY = 'yanime_history';
const PROGRESS_KEY = 'yanime_progress';
